...
class CyberSourceSoapAPI extends AbstractConnection
{
    ...
    const VISA_PAYMENT_SOLUTION = 'visacheckout';
     ...
     * @throws \Exception
     */
    public function reverseOrderPayment($storeId, $isVisaCheckout = false)
    {
        $request = new \stdClass();
        $request->partnerSolutionID = RequestDataBuilder::PARTNER_SOLUTION_ID;
        $developerId = $this->config->getValue(
        ...
        if ($isVisaCheckout) {
            $request->paymentSolution = self::VISA_PAYMENT_SOLUTION;
            $vc = new \stdClass();
            $vc->orderID = $this->payment->getAdditionalInformation("callID");
            $request->vc = $vc;
        }
