	...
	*/
    public function jwkToPem($jwkArray)
    {
		...
		if($this->productMetadata->getVersion() >= '2.4.4'){
			...
			$bigIntegerFactory = $objectManager->get('\phpseclib\Math\BigIntegerFactory');
			$RSAFactory = $objectManager->get('\phpseclib\Crypt\RSAFactory');
			
			$rsa = $RSAFactory->create();

			$exponent = $bigIntegerFactory->create([
